...
を定期的に実施した上で「これ以上の改善は見込めない」場合の参考にする。

また、詳細には踏み込まず「紹介程度」にとどめる。
...
データを RAM 上に保持することで、高速データアクセスを実現する方法だ。
...
データを複数のサーバーに複製し、読み取り操作を分散させる手法だ。
...
大規模なデータセットを複数のDBサーバー(シャード)に水平分割する手法だ。

データとクエリ負荷を分散させ、大規模なスケールアウトが可能になる。

テーブル分割作業も踏まえるため、他の手法と比較して難易度は高めだ。
...
手法はそれぞれ特性や難度が異なるため、要件、制約、要求技術に応じて適切な方式を選択することが重要だ。
