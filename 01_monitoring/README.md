...
本記事では、DB モニタリングの重要性とその基礎概念を記述する。

プロジェクトにモニタリングツール未導入の場合は参考にする。

## DB モニタリングツールの重要性

DBモニタリングツールは「**高信頼性と経済性を両立したシステム管理**」を目的とする。
...
プロジェクトの予算・目的・人員リソース等に応じて、何を導入するか検討する。
...
以下のような語句を用いて Web 検索を試し、予算や目的に収まるものを選定する。
...
閾値はあくまで一般的な目安だ。実際の構成要件などに応じて見定める。
...
効果的なアラート設定には、次の表に記した原則への考慮が重要だ。
